...
public class ItemSearchServiceImpl implements ItemSearchService {
    ...
    public Map search(Map searchMap) {
        ...
        if (!"".equals(searchMap.get("category"))){
            resultMap.putAll(searchBrandtAndSpecList((String) searchMap.get("category")));
        }else {
            if (categoryList.size()>0){
                resultMap.putAll(searchBrandtAndSpecList(categoryList.get(0)));
            }
        }
    ...
    private Map searchList(Map searchMap){
        ...
        for (HighlightEntry<TbItem> highlightEntry : highlighted) { //遍历高亮入口对象
            TbItem item = highlightEntry.getEntity(); //得到实体类
     ...
     * @return
     */
    private Map searchBrandtAndSpecList(String category){
        Map map = new HashMap();
        //根据商品分类名称得到模板id
